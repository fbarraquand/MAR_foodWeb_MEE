...
### Estimation of MAR/VAR matrix model

DBall=read.csv("/home/frederic/Documents/MAR_modelling/resubmission/MAR_foodWeb/LVR_foodWeb/LVR_simulation/DataFoodWeb_wTime_abs.csv") 
head(DBall)
nrow(DBall)
...
head(DB)
DB=DB[DB$Time_index %in% 801:900,]
head(DB) ## select only 100 points between 800 and 900
...
for (i in 1:length(sp)){
  ...
  for (j in 1:length(sp)){
    ...
    if(interaction_matrix[i,j]!=0){
      ...
      B1[i,j]=paste(s2,s,sep="") ### beware, was paste(s2,s,sep="") in Coralie's code but shouldn't this be the other way around? 
    }else{
      if(s==s2){
...
#############################################################################################
#### Code to extract the B matrix -- adapted from doi: https://doi.org/10.1101/171264 
#############################################################################################

Bnew=matrix(0,length(sp),length(sp))
sp=dimnames(fit_log$model$data)[[1]] #Names of your species
nom=dimnames(fit_log$par$B)[[1]] #B matrix coefficients
Bnew=matrix(0,length(sp),length(sp))
Blower=matrix(0,length(sp),length(sp))
Bupper=matrix(0,length(sp),length(sp))
...
### Try something else -- Gregoire's code // would be nice if I could get it to match
...
Cnew<-Cupper<-Clower<-rep(0,nspecies)
l=0
  for (i in 1:length(sp)){
    l=l+1
    if(!is.na(cis$par.se$U[l])){
      Cnew[i]=fit_log$par$U[l]### Shouldn't this be fit_log$par$C[l]? 
      ### I know we specify C=C1 and U="zero", and we get back the params in U... the results are correct but weirdly presented
      Cupper[i]=cis$par.upCI$U[l]
      Clower[i]=cis$par.lowCI$U[l]
    }
}
